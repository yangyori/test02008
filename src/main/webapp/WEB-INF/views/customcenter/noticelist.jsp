...
<link rel="stylesheet" href="/ex/resources/css/공지사항.css?var=7">
					...
					<table class="right_middle_table">
						...
						<c:forEach items="${list}" var="noticedto">
							<tr>
								...
								<td style="text-align: left;"><a href="/ex/customcenter/noticecontent">${noticedto.title}</a></td>
					...
					<div style="text-align: center;">
						<div class="pagination" style="padding: 60px 0 20px 0;">
							<c:if test="${pageMaker.page !=1}">
								<a href='noticelist${pageMaker.makeSearch(1)}'>&laquo;</a>
							</c:if>
							<c:if test="${pageMaker.prev }">
								<a
									href='noticelist${pageMaker.makeSearch(pageMaker.startPage-1)}'>&lt;</a>
							</c:if>

							<c:forEach begin="${pageMaker.startPage }"
								end="${ pageMaker.endPage}" var="idx">
								<a href='noticelist${pageMaker.makeSearch(idx)}'
									<c:out value="${pageMaker.page==idx?' class=active ':'' }"/>>
									${idx}</a>
							</c:forEach>

							<c:if test="${pageMaker.next }">
								<a href='noticelist${pageMaker.makeSearch(pageMaker.endPage+1)}'>&gt;</a>
							</c:if>


							<c:if test="${pageMaker.page != pageMaker.totalEndPage}">
								<a
									href='noticelist${pageMaker.makeSearch(pageMaker.totalEndPage)}'>&raquo;</a>
							</c:if>
